package com.dream.llb.common.widget;
...
/**
 * 封装的PopupWidow的工具类
 * 调用很简单，直接new完传递参数，实现方法，通过builder调用方法，可以自己扩展
 */
public abstract class PopupUtil {
     ...
     * @param parentView  a parent view to get the android.view.View.getWindowToken() token from
     * @param gravity     the gravity which controls the placement of the popup window
     * @param x           the PopupWindow's x location offset
     * @param y           the PopupWindow's y location offset
     */
    public PopupUtil(Context context, int layoutResId, int width, int height,
                     View parentView, int gravity, int x, int y, ClickListener callback) {
        ...
        PopBuilder builder = PopBuilder.createPopupWindow(context, layoutResId, width, height,
                parentView, gravity, x, y, mCallback);
    }
